...
export default function Profile() {
  ...
  useEffect(() => {
    const fetchMyPosts = async () => {
      try {
        ...
        if (myId) {
          const myPosts = data.filter(post => (post.user?._id || post.user) === myId);
  ...
  if (!currentUser) return <div className="min-h-screen bg-pix-black flex items-center justify-center"><Loader2 className="animate-spin text-pix-gold" /></div>;
  ...
  return (
    <div className="min-h-screen bg-pix-black text-white px-4 py-12">
        ...
        <div className="flex flex-col md:flex-row items-center gap-10 md:gap-20 border-b border-white/5 pb-16">
          ...
          <div className="flex-1 text-center md:text-left space-y-6">
            <div className="flex flex-col md:flex-row items-center gap-6">
              <h2 className="text-3xl font-black tracking-tight uppercase">{currentUser.username}</h2>
            ...
            <p className="text-sm text-white/90 italic">"{currentUser.bio || "Crafting visual stories."}"</p>
          </div>
        </div>
      ...
      {/* MODAL CHI TIẾT - DÙNG CHUNG CẢ APP */}
      {selectedPost && (
        <PostModal 
          post={selectedPost} 
          onClose={() => setSelectedPost(null)} 
          onCommentSuccess={(newComments) => {
             // Cập nhật lại list bài viết để số lượng comment hiển thị ở Grid khớp với thực tế
